...
This repository is best understood as a research workflow under active development, not yet as a fully packaged reproducible analysis. Before rerunning the project, users should check the following:

- replace local absolute file paths with project-relative paths;
- confirm that required raw data and intermediate files are available;
- confirm that the Stan models compile under the installed `rstan` version;
- check whether model outputs were generated from single-chain or multi-chain runs;
- set random seeds consistently for clustering, Stan sampling, and posterior summaries;
- document the number of topics, number of tiles, number of chains, and number of iterations used in each analysis.

## Citation and Acknowledgements

The LDA Stan implementation in `R_notebooks/06_LDA_scripts/06_lda.stan` notes that it was adapted from code by Kris Sankaran:

```text
https://github.com/krisrs1128/boot_expers/blob/master/ldaSim/inst/extdata/lda.stan
```

If this repository is used in academic work, please cite the relevant data source, the original LDA/Stan implementation where appropriate, and this repository.

## License

No license file is currently included. If this repository is intended for public reuse, add a `LICENSE` file to clarify reuse, modification, and distribution permissions.
